...
CREATE OR REPLACE FUNCTION increment_call_attempts(target_company_id TEXT)
RETURNS void
LANGUAGE plpgsql
SECURITY INVOKER
AS $$
BEGIN
  ...
  WHERE id = target_company_id
    AND (assigned_sales_id = auth.uid()::text OR auth.uid() IS NULL);
END;
$$;

CREATE OR REPLACE FUNCTION decrement_call_attempts(target_company_id TEXT)
RETURNS void
LANGUAGE plpgsql
SECURITY INVOKER
AS $$
