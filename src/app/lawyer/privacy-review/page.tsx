...
import { CheckCircle2, Clock, ArrowLeft, Scale, MessageSquare } from 'lucide-react';
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: '2px solid #e5e7eb' }}>
            ...
            <div style={{ padding: '16px 18px', background: hasIssue ? '#f0fdf4' : '#fafafa', borderLeft: '4px solid #16a34a' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    ...
                    <span style={{ marginLeft: 'auto', fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 20, background: '#dcfce7', color: '#166534', flexShrink: 0 }}>✅ 수정</span>
                </div>
    ...
    if (confirmed) return (
        <div style={{ minHeight: '100vh', background: '#f9fafb', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ textAlign: 'center', background: '#fff', borderRadius: 20, padding: '48px 64px', boxShadow: '0 4px 32px rgba(0,0,0,0.08)' }}>
                ...
                <Link href="/employee">
                    <button style={{ background: '#1d4ed8', color: '#fff', border: 'none', borderRadius: 10, padding: '12px 28px', fontWeight: 700, cursor: 'pointer', fontSize: 14 }}>
                        영업 리드 목록 →
                    </button>
                </Link>
            </div>
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#f3f4f6', fontFamily: "'Pretendard','Apple SD Gothic Neo',sans-serif" }}>
            ...
            <div style={{
                ...
                background: '#1e3a8a', borderBottom: '3px solid #2563eb',
                padding: '0 24px', height: 58,
                display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    <Link href="/employee">
                        <button style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'rgba(255,255,255,0.1)', color: '#e0e7ff', border: 'none', borderRadius: 7, padding: '5px 12px', cursor: 'pointer', fontSize: 13 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                    ...
                    {/* 지시수 배지 */}
                    {noteN > 0 && (
                        <span style={{ background: '#2563eb', color: '#fff', fontSize: 12, fontWeight: 700, borderRadius: 20, padding: '3px 10px' }}>
                            지시 {noteN}건
                        </span>
                    )}
                    {/* 컨펌 */}
                    <button onClick={handleConfirm} disabled={confirming} style={{
                        ...
                        background: confirming ? '#86efac' : '#16a34a', color: '#fff',
                        ...
                        {confirming ? '처리 중...' : '✅ 컨펌 & 발송'}
                    </button>
                </div>
            </div>

            {/* ── 컬럼 레이블 ─────────────────────────────────────── */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', background: '#1e40af', position: 'sticky', top: 58, zIndex: 90 }}>
                <div style={{ padding: '7px 20px', borderRight: '1px solid rgba(255,255,255,0.15)' }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: '#bfdbfe', letterSpacing: 1.5 }}>◀ 원문 + 이슈 / 시나리오</span>
                </div>
                <div style={{ padding: '7px 20px' }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: '#bbf7d0', letterSpacing: 1.5 }}>수정완료본 + 변호사 지시 ▶</span>
                </div>
            </div>
            ...
            {/* ── 변호사 최종 지시 + 컨펌 ────────────────────────── */}
            <div style={{ maxWidth: 880, margin: '32px auto 48px', background: '#fff', borderRadius: 16, padding: '28px 32px', boxShadow: '0 4px 24px rgba(0,0,0,0.07)', border: '2px solid #bfdbfe' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                    <Scale size={19} color="#1d4ed8" />
                    <span style={{ fontSize: 17, fontWeight: 900, color: '#1e3a8a' }}>변호사 최종 지시</span>
                    <span style={{ fontSize: 12, color: '#9ca3af', marginLeft: 'auto' }}>영업팀 전달 · 이메일 자동 발송</span>
                </div>

                {/* 항목별 지시 요약 */}
                {noteN > 0 && (
                    <div style={{ marginBottom: 14, background: '#eff6ff', borderRadius: 8, padding: '12px 16px', border: '1px solid #bfdbfe' }}>
                        <div style={{ fontSize: 11, fontWeight: 700, color: '#1d4ed8', marginBottom: 8 }}>📋 항목별 지시 요약 ({noteN}건)</div>
                        {Object.entries(notes).filter(([, v]) => v.trim()).map(([num, val]) => (
                            <div key={num} style={{ fontSize: 13, color: '#374151', marginBottom: 5, lineHeight: 1.5 }}>
                                <strong style={{ color: '#1e40af' }}>[{num}]</strong> {val}
                            </div>
                        ))}
                    </div>
                )}
                ...
                <textarea
                    ...
                    rows={4}
                    style={{ width: '100%', resize: 'vertical', fontSize: 14, color: '#111827', border: '2px solid #bfdbfe', borderRadius: 10, padding: '12px 16px', background: '#f8faff', outline: 'none', fontFamily: 'inherit', lineHeight: 1.7, boxSizing: 'border-box', marginBottom: 20 }}
                />
                <button onClick={handleConfirm} disabled={confirming} style={{
                    width: '100%', padding: '18px', borderRadius: 12,
                    background: confirming ? '#86efac' : 'linear-gradient(135deg,#16a34a,#15803d)',
                    color: '#fff', border: 'none', fontWeight: 900, fontSize: 17, cursor: confirming ? 'not-allowed' : 'pointer',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
                    boxShadow: '0 4px 20px rgba(22,163,74,0.35)',
                }}>
                    <CheckCircle2 size={22} />
                    {confirming ? '처리 중...' : '컨펌 & 영업팀 + 이메일 자동 발송'}
                </button>
            </div>
        </div>
    );
