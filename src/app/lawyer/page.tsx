        ...
        <div className="flex h-full" style={{ background: '#f8f9fc' }}>
            ...
            {sel && (
                <div className="flex-1 overflow-y-auto p-6 space-y-5">
                    ...
                    {sel.aiDraft ? (
                        <div>
                            <div className="flex items-center gap-2 mb-2"><Bot className="w-4 h-4" style={{ color: '#6366f1' }} /><span className="text-sm font-bold" style={{ color: '#6366f1' }}>1차 분석</span></div>
                            <div className="p-4 rounded-xl text-sm" style={{ background: '#eef2ff', color: '#4338ca', border: '1px solid #c7d2fe' }}>{sel.aiDraft}</div>
                        </div>
                    ) : <div className="p-4 rounded-xl text-center text-sm" style={{ color: '#94a3b8', background: '#f8f9fc', border: '1px solid #e5e7eb' }}>분석 준비 중...</div>}
                    {!done ? (
                        <div>
                            ...
                            <textarea value={note} onChange={e => setNote(e.target.value)} rows={6}
                                placeholder="초안을 수정하거나 직접 답변을 작성하세요..."
                            ...
                            {sel.aiDraft && !note && (
                                <button onClick={() => setNote(sel.aiDraft)} className="text-xs mt-1 flex items-center gap-1" style={{ color: '#94a3b8' }}>
                                    <Edit3 className="w-3 h-3" /> 초안 가져오기
                                </button>
                            )}
