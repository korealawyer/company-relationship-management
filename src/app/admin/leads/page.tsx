...
function SlidePanel({ lead, onClose, onUpdate }: { lead: Lead; onClose: () => void; onUpdate: () => void }) {
    const [tab, setTab] = useState<'contact' | 'email' | 'clause' | 'timeline' | 'memo'>('contact');
...
export default function LeadsPage() {
    ...
    const openPanel = (lead: Lead) => setActivePanel(lead);
                            ...
                            <div className="flex gap-3 flex-wrap">
                                <div className="relative flex-1 min-w-48">
                                    ...
                                    <input value={search} onChange={e => setSearch(e.target.value)}
                                        ...
                                        className="input-white pl-9 text-base" style={{ background: '#f8fafc' }} />
                                </div>
                                <select value={filterStatus} onChange={e => setFilterStatus(e.target.value as LeadStatus | 'all')}
                                    className="input-white w-auto px-3 text-base" style={{ background: '#f8fafc' }}>
