  ...
  "issues": [
    {
      ...
      "originalText": "법령 요건과 불일치하거나 문제 소지가 있는 원문 발췌 내용",
      "riskDesc": "법령 위반으로 인해 당장 발생할 수 있는 구체적 누락 항목 설명 및 기업이 맞닥뜨릴 막대한 제재 수위(예: 매출액 3% 과징금 등)와 치명적 리스크를 단호하고 강하게 경고",
      "scenario": "해당 조항 위반 시 기업에 즉각적으로 타격을 줄 수 있는 구체적이고 치명적인 위반 시나리오. 단순히 객관적 서술에 그치지 않고, 고객이 심각성을 확실하게 느낄 수 있도록 강력하게 경고 (예: 집단 소송 가능성 등)",
      "penalty": "위반 시 부과되는 구체적 법정 제재(과태료/과징금 등) 기준액 및 행정처분 명시 (예: '과징금 최대 55억 원')",
...
(각 조문 검토 진단 결과 리스트. 법리적으로 문제가 있는 항목은 (※ 상세 코멘트)를 부기하여 구체적이고 풍부하게 설명하십시오.)
...
## 8. 맺음말
본 실사 보고서는 귀사가 제공한 현행 문구를 기준으로 한 1차 진단으로, 특히 언급된 **치명적 결함 조항** 들은 법적 분쟁 시 즉각적인 제재 사유가 될 수 있는 아킬레스건입니다. 귀사에서는 상기 진단에 기반해 신속하게 내부 사실관계를 확정하신 후, 전문 법무법인을 통한 방침의 근본적이고 전면적인 개정 절차에 즉각 착수하실 것을 강력히 권고합니다.
[One-Shot 템플릿 종료]
...
const STORAGE_KEY = 'ibs_privacy_prompts_v3';
