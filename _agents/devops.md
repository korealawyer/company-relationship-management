...
## 모니터링 알림 기준:
  Error Rate > 1%/시간          → Slack 즉시 알림
  API 응답 > 3초               → 성능팀 알림
  DB 연결 > 80% max_connections → 경고
  RLS 위반 에러 감지            → 보안팀 즉시 에스컬레이션
  로그인 실패 5회 연속          → 계정 잠금 + 알림
...
## Supabase 마이그레이션 규칙:
  ...
  ✅ 모든 마이그레이션 supabase/schema.sql 반영 필수
