...
import {
    ...
    Clock, Eye, Star, Bot, FileEdit, CreditCard,
} from 'lucide-react';
...
function ActionButton({
    ...
    if (s === 'lawyer_confirmed') return (
        <div className="flex items-center gap-1.5">
            <Button variant="premium" size="sm" onClick={() => router.push(`/admin/email-preview?leadId=${c.id}&company=${encodeURIComponent(c.name)}`)}>
                <FileEdit className="w-3.5 h-3.5 mr-1" /> 이메일 작성
            </Button>
        </div>
    );
    if (s === 'emailed') return (
    ...
    if (s === 'client_replied') {
        const handlePaymentCheck = async () => {
            try {
                ...
                if (data.found) {
                    store.update(c.id, { paymentVerified: true, paymentVerifiedAt: data.payment.paidAt });
                    refresh();
                    alert(`✅ 결제 확인 완료\n\n금액: ${data.payment.amount.toLocaleString()}원\n방법: ${data.payment.method}\n일시: ${new Date(data.payment.paidAt).toLocaleString('ko-KR')}\n거래번호: ${data.payment.txId}\n\n이제 '구독 처리' 버튼을 클릭해주세요.`);
                } else {
                    alert(`❌ 결제 내역 없음\n\n${data.message}`);
                }
            } catch {
        ...
        return (
            <div className="flex items-center gap-1.5">
                {!c.paymentVerified ? (
                    <Button variant="outline" size="sm" onClick={handlePaymentCheck} disabled={loading === c.id}>
                        <CreditCard className="w-3.5 h-3.5 mr-1" />
                        {loading === c.id ? '확인 중...' : '결제확인'}
                    </Button>
                ) : (
                    <span className="text-[10px] flex items-center gap-1 font-bold" style={{ color: '#16a34a' }}>
                        <CheckCircle2 className="w-3 h-3" /> 결제완료
                    </span>
                )}
                <Button variant="premium" size="sm"
                    disabled={!c.paymentVerified}
                    onClick={() => run(c.id, () => store.update(c.id, { plan: 'standard', status: 'subscribed' }))}
                    style={!c.paymentVerified ? { opacity: 0.4, cursor: 'not-allowed' } : {}}>
        ...
        <div className="min-h-screen px-4 py-8 max-w-[1600px] mx-auto" style={{ background: T.bg }}>
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-sm mt-0.5" style={{ color: T.muted }}>AI 개인정보처리방침 분석 → 영업 컨펌 → 변호사 검토 → 이메일 발송 | {companies.length}개 기업 진행 중</p>
                </div>
