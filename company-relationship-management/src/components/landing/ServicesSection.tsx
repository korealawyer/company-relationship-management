            ...
            <div className="max-w-6xl mx-auto">
                <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="text-center mb-16">
                    ...
                    <h2 className="text-3xl sm:text-4xl font-black mb-4 text-light">
                        IBS 법률사무소가 <span className="text-gold-gradient">5가지를 기본 제공</span>합니다
                    </h2>
                    <p className="text-muted-60">단순 법률자문이 아닌 — 본사·가맹점·임직원 전체를 커버하는 브랜드 리스크 운영 예산</p>
                </motion.div>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                        ...
                        <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
                            ...
                            <Card padding="lg" className="h-full">
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <div>
                                        <p className="font-black text-sm text-light">{svc.title}</p>
                                        <p className="text-xs text-muted-40">{svc.sub}</p>
                                    </div>
                                </div>
                                <p className="text-sm leading-relaxed mb-3 text-muted-65">{svc.desc}</p>
                    ...
                    <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
                        ...
                        <Card padding="lg" gold className="h-full flex flex-col justify-between">
                            <div>
                                <p className="text-xs font-bold mb-2 text-gold-80">핵심 차별점</p>
                                {['사건 처리 → 사고 예방 중심', '본사 + 가맹점 + 임직원 원스톱', '공식 법률파트너 표기 제공', '분기 리스크 브리핑으로 선제 대응'].map((pt, j) => (
                                    <div key={j} className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className="text-sm text-muted-80">{pt}</span>
                                    </div>
                                ))}
                            </div>
                            <div className="mt-4 pt-4" style={{ borderTop: '1px solid rgba(201,168,76,0.2)' }}>
                                <p className="text-xs text-muted-40">최소 계약: 1년 / 분기별 가맹점 수 기준 요금 리베이스</p>
                            </div>
                        </Card>
                    </motion.div>
                </div>
