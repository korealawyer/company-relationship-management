        ...
        <section className="py-24 px-4 sm:px-6 lg:px-8">
            <div className="max-w-6xl mx-auto">
                <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="text-center mb-16">
                    ...
                    <p className="text-muted-60">실제 발생한 프랜차이즈 법적 분쟁 사례를 기반으로 한 리스크 시나리오</p>
                </motion.div>
                    ...
                    {RISK_SCENARIOS.map((scenario, i) => (
                        <motion.div key={i} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }}
                            ...
                            <Card padding="lg" className="h-full">
                                ...
                                <h3 className="font-black text-lg mb-3 text-light">{scenario.title}</h3>
                                <p className="text-sm leading-relaxed text-muted-65">{scenario.desc}</p>
                            </Card>
                        </motion.div>
                    ))}
                </div>
