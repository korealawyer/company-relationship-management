    ...
    return (
        <div>
            <button
                ...
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); open ? setOpen(false) : openMenu(); } }}
...
export default function Navbar() {
    ...
    // AuthContext의 user를 단일 소스로 사용 (자체 useState 제거)
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-20">
                    ...
                    <Link href={user ? (LINKS_BY_ROLE[user.role]?.[0]?.href ?? '/') : '/'} className="flex items-center gap-2.5 group flex-shrink-0">
                        <div>
                            <span className="font-black text-2xl leading-tight block"
                    ...
                    {/* 모바일 햄버거 */}
                    <button
                        className="lg:hidden p-2 rounded-lg" style={{ color: '#c9a84c' }}
